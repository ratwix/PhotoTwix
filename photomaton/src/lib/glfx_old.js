...
function getPixelArray() {
	if (getPixelArray.arguments.length != 4) {
		...
		var x = toDataURL.arguments[1];
		var y = toDataURL.arguments[2];
		var w = toDataURL.arguments[3];
		var h = toDataURL.arguments[4];
...
function toDataURL(mimeType) {
	...
	if (toDataURL.arguments.length == 5) { //We do not want to capture entire, pass 4 more argument : x, y, w, h
		x = toDataURL.arguments[1];
		y = toDataURL.arguments[2];
		w = toDataURL.arguments[3];
		h = toDataURL.arguments[4];
	}

    var array = (toDataURL.arguments.length == 5) ? getPixelArray.call(this, x, y, w, h) : getPixelArray.call(this);
